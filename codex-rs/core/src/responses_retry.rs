...
pub(crate) async fn handle_retryable_response_stream_error(
    ...
    if *retries < max_retries {
        *retries += 1;
        ...
        // Hide the first websocket retry notification to reduce noisy transient reconnect
        // messages. Retry details remain available in logs for diagnosis.
        let report_error =
            retry_count > 1 || !sess.services.model_client.responses_websocket_enabled();
        if report_error {
