...
fn quoted_external_editor_shortcut_can_be_overridden_or_unbound() {
    ...
    config.global.open_external_editor_with_quote = Some(bindings(&[]));
    let unbound = RuntimeKeymap::from_config(&config).expect("valid quoted editor unbinding");
    assert!(unbound.app.open_external_editor_with_quote.is_empty());
}

#[test]
fn configured_single_binding_can_shadow_the_default_quoted_editor_shortcut() {
    let mut config = TuiKeymap::default();
    config.global.copy = Some(binding("alt-g"));
    ...
    assert_eq!(runtime.app.copy, vec![key_hint::alt(KeyCode::Char('g'))]);
}

#[test]
fn configured_chord_prefix_can_shadow_the_default_quoted_editor_shortcut() {
    let mut config = TuiKeymap::default();
    config.global.copy = Some(binding("alt-g ctrl-e"));
    ...
    assert_eq!(
        ...
        Some(["alt-g ctrl-e".to_string()].as_slice())
    );
}
