...
async fn status_line_hostname_items_render_short_and_full_hostnames() {
    let (mut chat, _rx, _ops) = make_chatwidget_manual(/*model_override*/ None).await;
    let host = codex_config::os_host_name();
    let expected = host.as_ref().map_or((None, None), |host| {
        let hostname = host
            .split_once('.')
            .map_or(host.as_str(), |(hostname, _domain)| hostname)
            .to_string();
        (Some(hostname), Some(host.clone()))
    });
...
#[tokio::test]
async fn status_line_host_renders_current_machine_hostname() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.thread_id = Some(ThreadId::new());
    chat.config.tui_status_line = Some(vec!["host".to_string()]);
    ...
    assert!(
        drain_insert_history(&mut rx).is_empty(),
        "host should be accepted as a status line item"
    );
}
