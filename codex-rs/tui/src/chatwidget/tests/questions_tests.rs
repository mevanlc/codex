...
async fn accepted_question_answer_uses_existing_delivery_and_keeps_main_draft() {
    for queued in [false, true] {
        ...
        if queued {
            ...
            let restored = chat.pop_latest_locally_queued_composer_state().unwrap();
...
#[tokio::test]
async fn question_navigation_can_retract_a_pending_steer() {
    let (mut chat, _rx, mut ops) = make_chatwidget_manual(/*model_override*/ None).await;
    let pending = pending_steer("pending follow-up");
    ...
    chat.handle_key_event(forward);
    insta::assert_snapshot!(
        "last_question_with_pending_steer",
        render_bottom_popup(&chat, /*width*/ 80)
    );
    chat.handle_key_event(forward);
    assert_eq!(
        ops.try_recv().expect("retract pending steer"),
        Op::RetractSteer {
            expected_turn_id: pending.turn_id.clone().unwrap(),
            client_user_message_id: pending.client_id.clone(),
        }
    );
    assert!(!chat.bottom_pane.questions.as_ref().unwrap().expanded);
    assert_eq!(
    ...
    assert_eq!(chat.bottom_pane.composer_text(), "pending follow-up");
    assert_eq!(question_count(&chat), 2);
