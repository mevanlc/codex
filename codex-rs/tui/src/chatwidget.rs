...
pub(crate) struct ChatWidget {
    ...
    /// Keybinding to show for popping the most-recently queued or pending
    /// follow-up message back into the composer. This may differ from the first
    /// configured binding when the default set includes a terminal-specific fallback.
...
impl ChatWidget {
    ...
    fn pop_latest_retractable_user_message(&mut self) -> Option<UserMessage> {
        ...
        } else if let Some(pending) = self.input_queue.pending_steers.pop_back() {
            Some(user_message_for_restore(
    ...
    fn new_with_op_target(common: ChatWidgetInit, codex_op_target: CodexOpTarget) -> Self {
        ...
        let side_placeholder = select_side_chatbox_placeholder(&config, &mut rng);
...
fn select_chatbox_placeholder<R: Rng + ?Sized>(config: &Config, rng: &mut R) -> String {
    select_placeholder_from(&PLACEHOLDERS, config, rng)
}

fn select_side_chatbox_placeholder<R: Rng + ?Sized>(config: &Config, rng: &mut R) -> String {
    select_placeholder_from(&SIDE_PLACEHOLDERS, config, rng)
}

fn select_placeholder_from<R: Rng + ?Sized>(
    placeholders: &[&str],
    config: &Config,
    rng: &mut R,
) -> String {
